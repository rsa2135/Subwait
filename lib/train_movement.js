    ...
    // $('#start-button').click(function() {
      setTimeout(function() {
        ...
        for (let i = 0; i < totalTrains; i++) {
          setTimeout(callback1(south, stops, i), 50);
        }
      }, 0);
  });
});
  ...
  // Start iterating over the path's elements and reveal each one (through opacity)
    function iterate(data) {
        ...
        path.transition()
            .delay(function(d) {
              if (d.properties.stationnumber === "0") {
                ...
                const finish = moment(d.properties[`arrival${j}`], 'HH:mm:ss');
            ...
            .duration(function(d) {
              console.log(`i is: ${i}`);
              console.log(`j is: ${j}`);
              const start = moment(d.properties[`arrival${j}`], 'HH:mm:ss');
              const finish = moment(d.properties[`arrival${j + 1}`], 'HH:mm:ss');
              const duration = moment.duration(finish.diff(start)).as('milliseconds');
              const durationMinutes = (duration); // 60000;
              return (durationMinutes);// * (1/timeFactor) * 1000);
            })
      ...
      function tweenDash(d) {
        ...
        return function (t) {
          let marker1 = d3.select("#marker" + j);
